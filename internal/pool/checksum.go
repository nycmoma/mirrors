...
// Complete reports whether all currently tracked checksums are present.
func (c Checksum) Complete() bool {
	return c.MD5 != "" && c.SHA1 != "" && c.SHA256 != "" && c.SHA512 != "" && c.Size >= 0
}

func checksumsForFile(path string) (Checksum, error) {
...
func verifyExpected(path string, actual Checksum, expected Checksum) error {
	if expected.Size >= 0 && expected.Size != actual.Size {
		return fmt.Errorf("%s: size mismatch %d != %d", path, actual.Size, expected.Size)
	}
	if expected.MD5 != "" && !sameHash(actual.MD5, expected.MD5) {
		return fmt.Errorf("%s: md5 mismatch %q != %q", path, actual.MD5, expected.MD5)
	}
	if expected.SHA1 != "" && !sameHash(actual.SHA1, expected.SHA1) {
		return fmt.Errorf("%s: sha1 mismatch %q != %q", path, actual.SHA1, expected.SHA1)
	}
	if expected.SHA256 != "" && !sameHash(actual.SHA256, expected.SHA256) {
		return fmt.Errorf("%s: sha256 mismatch %q != %q", path, actual.SHA256, expected.SHA256)
	}
	if expected.SHA512 != "" && !sameHash(actual.SHA512, expected.SHA512) {
		return fmt.Errorf("%s: sha512 mismatch %q != %q", path, actual.SHA512, expected.SHA512)
	}
	return nil
}
